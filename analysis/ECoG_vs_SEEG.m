...
figure(2);clf
subplot(1,2,1)
histogram(ecog_distances_all,'Normalization','probability')
xlim([0 200])
ylabel('Frequency')
xlabel('Internodal distance (mm)')
title('ECoG internodal distances')
subplot(1,2,2)
histogram(seeg_distances_all,'Normalization','probability')
xlim([0 200])
ylabel('Frequency')
xlabel('Internodal distance (mm)')
...
subplot(3,1,3)
hold on
histogram(seeg_conn_all(:,5),'Normalization','probability','FaceColor', color2)
histogram(ecog_conn_all(:,5),'Normalization','probability','FaceColor', color1)
xlim([0.1,0.6])
title('High-gamma coherence')
ylabel('Frequency')
...
boxplot([[sw_value_ecog(:,1);NaN],sw_value_seeg(:,1),[sw_value_ecog(:,3);NaN],...
    sw_value_seeg(:,3),[sw_value_ecog(:,5);NaN],sw_value_seeg(:,5)])
set(gca,'xtick',(1.5:2:5.5),'xticklabel',{'Broadband CC','Beta','High-Gamma'});
ylabel('Small-world propensity')
ylim([0.25 1.1])
...
ranksum(sw_value_ecog(:,5),sw_value_seeg(:,5))

signrank(sw_value_ecog(:,1),sw_value_ecog(:,3))
signrank(sw_value_ecog(:,1),sw_value_ecog(:,5))
signrank(sw_value_ecog(:,3),sw_value_ecog(:,5))

signrank(sw_value_seeg(:,1),sw_value_seeg(:,3))
signrank(sw_value_seeg(:,1),sw_value_seeg(:,5))
signrank(sw_value_seeg(:,3),sw_value_seeg(:,5))
...
[ECoG_module_purity] = network_similarity(ecog_pt_modules, {ecog_patients.coords}, 'systems');
[SEEG_module_purity] = network_similarity(seeg_pt_modules, {seeg_patients.coords}, 'systems');
...
ylim([-1.5 2])
subplot(1,3,3)
boxplot([[ecog_str_non_res(:,5);NaN],[ecog_str_res(:,5);NaN],seeg_str_non_res(:,5),seeg_str_res(:,5)])
set(gca,'xtick',(1:4),'xticklabel',{'ECoG non res','ECoG res','SEEG non res','SEEG res'});
ylabel('Z score node strength')
title('High-gamma coherence')
ylim([-1.5 2])
...
ranksum(ecog_str_non_res(:,5),ecog_str_res(:,5))

ranksum(seeg_str_non_res(:,1),seeg_str_res(:,1))
ranksum(seeg_str_non_res(:,3),seeg_str_res(:,3))
ranksum(seeg_str_non_res(:,5),seeg_str_res(:,5))
...
ylim([-0.5 2])
subplot(1,3,3)
boxplot([[ecog_btw_non_res(:,5);NaN],[ecog_btw_res(:,5);NaN],seeg_btw_non_res(:,5),seeg_btw_res(:,5)])
set(gca,'xtick',(1:4),'xticklabel',{'ECoG non res','ECoG res','SEEG non res','SEEG res'});
ylabel('Z score betweenness centrality')
title('High-gamma coherence')
ylim([-0.5 2])
...
ranksum(ecog_btw_non_res(:,5),ecog_btw_res(:,5))

ranksum(seeg_btw_non_res(:,1),seeg_btw_res(:,1))
ranksum(seeg_btw_non_res(:,3),seeg_btw_res(:,3))
ranksum(seeg_btw_non_res(:,5),seeg_btw_res(:,5))
...
ylim([-1 3])
subplot(1,3,3)
boxplot([[ecog_cc_non_res(:,5);NaN],[ecog_cc_res(:,5);NaN],seeg_cc_non_res(:,5),seeg_cc_res(:,5)])
set(gca,'xtick',(1:4),'xticklabel',{'ECoG non res','ECoG res','SEEG non res','SEEG res'});
ylabel('Z score control centrality')
title('High-gamma coherence')
ylim([-1 3])
...
ranksum(ecog_cc_non_res(:,5),ecog_cc_res(:,5))

ranksum(seeg_cc_non_res(:,1),seeg_cc_res(:,1))
ranksum(seeg_cc_non_res(:,3),seeg_cc_res(:,3))
ranksum(seeg_cc_non_res(:,5),seeg_cc_res(:,5))
